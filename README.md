...
**Buy-side pricing software for coin shops.** Photograph an incoming lot, apply the shop's buy rules, and issue one itemized purchase offer the customer can review and accept remotely.

CounterMint takes a dealer from intake to a defensible valuation, a branded customer quote, and a governed accept, counter, or decline workflow. It combines volatile metal markets, sold-comparable evidence, dealer policy, visual quote design, staff authority, and customer response history, and it keeps every provider, including any AI model, out of the role of price oracle.

**Founding-dealer preview opens 14 September 2026 · Designed, built, deployed, and operated by Raymond J. Kraft**

[countermint.io](https://countermint.io) · [View a live customer quote](https://app.countermint.io/q/YcQk3L7mU5mVaF5L-es1bGU8pUIZy7_JVTSQmR-gBOg) · [Request a walkthrough](mailto:ray@rootsnolimits.com)
...
|---:|---:|---:|---:|
| **86,120** lines | **186,916** lines in 195 files | **1,919** declared | **53** Alembic revisions |

<sub>Measured from the private product repository at commit `ea0a128` (2026-09-25); refreshed 2026-09-24. 283,262 lines of code in total across application, tests, migrations, and infrastructure; 263 commits since 2026-07-02, 148 in the last 30 days. Test cases: 1,472 pytest (249 parametrized) and 447 Node test-runner cases. Counting rules and the workflow that keeps this current: <a href="metrics/README.md">metrics/README.md</a>.</sub>
<!-- metrics:end -->
...
[![lines of code](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Frjk6831%2Fcountermint-architecture%2Fmain%2Fmetrics%2Fbadges%2Fcode.json)](metrics/README.md)
[![migrations](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Frjk6831%2Fcountermint-architecture%2Fmain%2Fmetrics%2Fbadges%2Fmigrations.json)](metrics/README.md)
[![measured](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Frjk6831%2Fcountermint-architecture%2Fmain%2Fmetrics%2Fbadges%2Fmeasured.json)](metrics/README.md)

A workflow writes these figures. On every push to the product's `main` branch, [`metrics/collect.mjs`](metrics/collect.mjs) counts the private repository at that commit and [`metrics/render.mjs`](metrics/render.mjs) rewrites the table above. The counting rules are public and reproducible: [metrics/README.md](metrics/README.md). The marketing site at countermint.io is a separate Cloudflare Worker and is not included in these counts.
...
<p align="center"><em>This is the actual quote renderer. Clicking the header opens the fields bound to that section of the design JSON.</em></p>
...
|---|---|---|
| **Appraisal intake** | Manual entry, multi-item image capture, label extraction, and distinct catalog identifiers | Uncertain fields are staged for confirmation; nothing is guessed on the dealer's behalf |
...
| **Metals Insights** | Four core quote metals plus 16 additional instruments, with interactive native line and candlestick charts, ranges, intervals, freshness, and OHLC tables | One-minute-delayed provider observations; no invented buckets and no live-trading claim |
...
| **Quote Studio** | Presets, click-to-edit regions, brand controls, artwork composition, customer actions, and responsive preview | Strict versioned design JSON and one shared customer, preview, and print renderer |
| **Customer negotiation** | Accept, customer counter, dealer counter-back, rejection, and terminal per-item decline | State-checked server lifecycle regardless of dealer-configurable labels and thresholds |
...
4. **Apply dealer policy.** Versioned ranges and Premium Protection operate on explicit inputs, so the dealer's strategy is visible in the rules and never hidden inside one opaque adjustment.
...
Quote Studio is a schema-backed editor around a versioned `QuoteDesignDocument` and a shared renderer. There is no free canvas, no screenshot generator, and no arbitrary CSS pasted onto customer data.
...
- **Start curated, then personalize.** Dealers can create from approved presets, duplicate designs, edit drafts, undo and redo, and preview desktop, mobile, and print before publishing.
...
- **Freeze what the customer received.** Publishing activates a versioned design, and quote issuance resolves the exact presentation and assets into an immutable presentation snapshot.
...
- empty buckets remain gaps and are never interpolated into false history.

This is delayed market context for dealer research. It is neither a live feed nor a trading tool. Dealers can switch instruments, line or candlestick presentation, date ranges, and sampling intervals, with accessible OHLC tables and freshness disclosures. The research surface remains separate from the authoritative melt path so chart interaction cannot rewrite a quote.
...
<p align="center">
  <a href="screenshots/quote-melt-premium-breakdown.png">
    <img src="screenshots/quote-melt-premium-breakdown.png" width="1000" alt="Earlier CounterMint UI showing separate melt value, collector premium, current comparable value, versioned dealer-policy range, and final offer">
  </a>
</p>

<p align="center"><em><strong>Earlier UI.</strong> The layout has changed; the contract has not: melt, collector premium, market evidence, the versioned policy range, and the final offer remain separate.</em></p>

---

## Customer negotiation, end to end

Customer negotiation is implemented as a state machine with audit history on every transition:
...
Quote Studio lets the dealer choose approved labels and whether terminal decline appears; presentation never redefines the server-owned lifecycle meaning.
...
The GSID image near the top of this README shows only the normal appraisal entry point. It is not a capture of the separate enhanced sandbox interface described here and should not be read as representative of it.
...
| Status | Scope |
|---|---|
| **Operational, ahead of the 14 September founding-dealer preview** | Appraisal intake, deterministic pricing, sold-comp provenance, versioned dealer rules, live customer quotes, customer and dealer counter workflows, staff roles, billing, and accepted-value history |
...
| **Implemented but capability- or provider-gated** | AI-assisted Quote Studio artwork and bounded style proposals; publisher-authorization-gated CDN integration boundary |
...
| **Roadmap** | Evidence-backed technical-analysis context, richer chart interaction, AI Rarity Insights, native mobile clients, and a cloud object-store adapter |
...
| A live market must not erase what the customer accepted | Freeze presentation and baseline spot on issuance, then append the accepted observation during the customer and dealer handshake | More storage and distinct open, accepted, reviewed, and confirmed paths to keep aligned | [ADR 0002](docs/decisions/0002-immutable-quote-snapshots.md) |
| Metered provider calls become expensive and inconsistent when scattered | Put every external pricing source behind one internal gateway | A new reliability boundary and explicit cache-staleness trade-offs | [ADR 0003](docs/decisions/0003-centralized-pricing-data-api.md) |
...
| Wrong code released | Exact-commit test receipts and post-deploy API and frontend revision verification |

The [engineering evidence note](docs/engineering-evidence.md) contains the measurement basis, the test-count boundary, the schema snapshot, and the release-evidence limits.
...
Add basic trend and momentum indicators over completed locally sampled candles, with versioned calculations, traceable inputs, coverage and freshness gates, expiry, and decision-support-only language. Signals will not feed the deterministic offer engine and will not represent executable trading advice.

### Richer market exploration

Extend the existing interactive line and candle, range, and interval experience with deeper comparison and inspection tools while preserving gaps and provider-delay disclosures.

### AI Rarity Insights

Add a tenant-scoped, allowance-metered workflow for requesting an in-depth review of one coin from a configured foundation model. Query allowances and provider budgets will scale by dealer subscription tier. Outputs will remain guarded, non-authoritative research: no authentication, grading, provenance, rarity certification, or pricing authority.
...
- Premium confidence is not yet surfaced as a first-class dealer control. Sparse comparable coverage remains visible and is never hidden behind false certainty.
...
Python 3.12 · FastAPI (async) · SQLAlchemy 2.0 · Alembic · PostgreSQL 16 · pytest · React 18 · Vite · Node test runner · Docker Compose · Caddy · DigitalOcean · Cloudflare Workers (marketing site) · Stripe
